...
import { replaceHtml, getObjType, chatatABC, ArrayUnique, showrightclickmenu, luckysheetactiveCell, luckysheetContainerFocus, $$, } from "../utils/util";
...
import { createExportDialog } from "../expendPlugins/exportXlsx/plugin";
...
export default function luckysheetHandler() {
  ...
  $("#luckysheet-exportXlsx-btn-title").click(function () {
    const exportXlsxInfo = Store.plugins.find(
      plugin => plugin.name === "exportXlsx"
    );
    if (exportXlsxInfo) {
      createExportDialog(exportXlsxInfo?.config?.url);
    } else {
