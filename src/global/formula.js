import { replaceHtml, getObjType, chatatABC, ABCatNum } from "../utils/util";
...
const luckysheetformula = {
  ...
  searchFunctionPosition: function ($menu, $editor, x, y, isparam) {
    ...
    if (y + menuH > winH) {
      ...
      if (!isparam) {
        $menu.html(
          $menu.find(".luckysheet-formula-search-item").get().reverse()
        );
      }
    }
  ...
  execFunctionGroup: function (
    ...
    if (!window.luckysheet_compareWith) {
      ...
      window.luckysheet_indirect_check_return =
        luckysheet_indirect_check_return;
      ...
      for (let i = 0; i < group.length; i++) {
        ...
        if (cell != null && cell.f != null && cell.f == calc_funcStr) {
          ...
          if (_this.isFunctionRangeSave) {
            stack.push(item);
            _this.execvertex["r" + item.r + "c" + item.c + "i" + item.index] =
              item;
            count++;
          }
        }
      ...
      if (getObjType(result.data) == "array") {
        ...
        if (getObjType(result.data) == "object" && !isRealNull(result.data.v)) {
          ...
          if (
            result.cell > 1 ||
            result.rowl > 1 ||
            getObjType(result.data) == "string" ||
            getObjType(result.data) == "number"
          ) {
            result = result.data;
